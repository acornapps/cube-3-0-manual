...
청지윈 public Hub는 공인인증서를 이용하여 Harbor &lt;-&gt; 청지윈 build server 간 통신 하게 된다.

청지윈 Private Hub는 Harbor Registry와 청지윈 build Server를 위한 인증서 Self signed certificate를 생성하여 내부에서

Harbor와 Docker 인증서를 생성한다.

![]({{ site.baseurl }}/assets/PrivateHubOverview.png)
