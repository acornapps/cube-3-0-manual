...
---
k8s cluster를 어떤 이유로 재설치 경우, etcd snapshot과 청지윈 cmdb, builderdb를 restore하여 기존 상태로 복원하는 절차를 기술한다.
...
**1.etcd, 청지윈 cmdb backup**

아래와 같이 etcd snapshot 생성, 청지윈 db backup을 위한 shell script를 자신의 환경에 맞게 수정한 후 backup을 주기적으로 실시한다.
```
